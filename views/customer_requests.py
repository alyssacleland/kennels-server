...
def get_customer_by_email(email):
    ...
    with sqlite3.connect("./kennel.sqlite3") as conn:
        ...
        for row in dataset:
            customer = Customer(
                row['id'], row['name'], row['address'], row['email'], row['password'])  # i think here is where we'd wanna exclude email and password bc default values so we don't have to include them and we don't wanna return them??
            customers.append(customer.__dict__)
